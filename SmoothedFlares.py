...
# N defines how many elements the moving average will average over. If it is set to 1, it is the same as not taking an average at all
N = 30

flarenb_moving_average = np.convolve(flarenumbers[:, 0], np.ones((N,))/N, mode='valid')
flareint_moving_average = np.convolve(flarenumbers[:, 1], np.ones((N,))/N, mode='valid')
spotnumbers = np.convolve(spotnumbers, np.ones((N,))/N, mode='valid')

# Normalising the data
flare_nb_max = max(flarenb_moving_average)
flare_intens_max = max(flareint_moving_average)
spot_max = max(spotnumbers)

flarenumbersNorm = np.zeros((len(flarenb_moving_average), 2))
flarenumbersNorm[:, 0] = flarenb_moving_average / flare_nb_max
flarenumbersNorm[:, 1] = flareint_moving_average / flare_intens_max
